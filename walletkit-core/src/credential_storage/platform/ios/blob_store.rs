...
mod tests {
    ...
    fn create_temp_store() -> (TempDir, IosBlobStore) {
        let temp_dir = TempDir::new().unwrap();
        let store = IosBlobStore::new(temp_dir.path()).unwrap();
        (temp_dir, store)
    }
    ...
    fn test_write_and_read() {
        ...
        store.write_atomic("test.bin", data).unwrap();

        let read_data = store.read("test.bin").unwrap().unwrap();
    ...
    fn test_overwrite() {
        ...
        store.write_atomic("test.bin", b"first").unwrap();
        store.write_atomic("test.bin", b"second").unwrap();

        let data = store.read("test.bin").unwrap().unwrap();
    ...
    fn test_delete() {
        ...
        store.write_atomic("test.bin", b"data").unwrap();
        assert!(store.exists("test.bin").unwrap());

        store.delete("test.bin").unwrap();
        assert!(!store.exists("test.bin").unwrap());
    }
    ...
    fn test_delete_nonexistent() {
        ...
        // Should not error
        store.delete("nonexistent.bin").unwrap();
    }
    ...
    fn test_empty_data() {
        ...
        store.write_atomic("empty.bin", b"").unwrap();

        let data = store.read("empty.bin").unwrap().unwrap();
    ...
    fn test_large_data() {
        ...
        store.write_atomic("large.bin", &data).unwrap();

        let read_data = store.read("large.bin").unwrap().unwrap();
        assert_eq!(read_data, data);
    }
    ...
    fn test_special_characters_in_filename() {
        ...
        // Note: Some special characters may not work on all file systems
        let filename = "test-file_123.bin";
        store.write_atomic(filename, b"data").unwrap();

        let data = store.read(filename).unwrap().unwrap();
    ...
    fn test_no_temp_file_left_on_success() {
        ...
        // Check that no temp file exists
        let temp_path = temp.path().join(".test.bin.tmp");
    ...
    fn test_creates_directory() {
        ...
        let store = IosBlobStore::new(&nested_path).unwrap();
        assert!(nested_path.exists());

        store.write_atomic("test.bin", b"data").unwrap();
        assert!(store.read("test.bin").unwrap().is_some());
    }
}
