...
mod tests {
    ...
    fn create_temp_store() -> (TempDir, IosVaultStore) {
        let temp_dir = TempDir::new().unwrap();
        let vault_path = temp_dir.path().join("account.vault");
        let store = IosVaultStore::new(&vault_path).unwrap();
        (temp_dir, store)
    }
    ...
    fn test_open_fails_if_not_exists() {
        ...
        let result = store.open_existing();
        assert!(result.is_err());
    }
    ...
    fn test_write_and_read_at() {
        ...
        let buf = store.read_at(0, data.len() as u32).unwrap();
    ...
    fn test_append() {
        ...
        let offset1 = store.append(b"first").unwrap();
        assert_eq!(offset1, 0);

        let offset2 = store.append(b"second").unwrap();
        assert_eq!(offset2, 5);
    ...
    fn test_write_at_extends_file() {
        ...
        // Write at offset 100 (beyond current end)
        store.write_at(100, b"data").unwrap();

        assert_eq!(store.len().unwrap(), 104);

        // Read back
        let buf = store.read_at(100, 4).unwrap();
    ...
    fn test_set_len_truncate() {
        ...
        let buf = store.read_at(0, 5).unwrap();
    ...
    fn test_sync() {
        ...
        store.sync().unwrap(); // Should not error
    }
    ...
    fn test_empty_operations() {
        ...
        store.write_at(0, b"").unwrap();
        assert_eq!(store.len().unwrap(), 0);

        // Empty read should succeed
        let buf = store.read_at(0, 0).unwrap();
        assert!(buf.is_empty());

        // Empty append should return current length
    ...
    #[test]
    fn test_reopen() {
        let (temp, store) = create_temp_store();

        // Create and write
        store.create_new().unwrap();
        store.write_at(0, b"persisted data").unwrap();
        store.sync().unwrap();

        // Drop the store
        drop(store);

        // Create new store for same path
        let vault_path = temp.path().join("account.vault");
        let store2 = IosVaultStore::new(&vault_path).unwrap();

        // Open existing file
        store2.open_existing().unwrap();

        // Read back data
        let buf = store2.read_at(0, 14).unwrap();
    ...
    fn test_creates_parent_directory() {
        let temp_dir = TempDir::new().unwrap();
        let vault_path = temp_dir.path().join("nested").join("dir").join("account.vault");
    ...
    fn test_open_or_create() {
        ...
        // Write some data
        store.write_at(0, b"test").unwrap();
        store.sync().unwrap();

        // Verify the store was created and is functional
        let data = store.read_at(0, 4).unwrap();
