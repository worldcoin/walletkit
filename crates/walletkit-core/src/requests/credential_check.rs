...
pub enum CredentialConstraintsCheckError {
    ...
    ConstraintTooLarge,
    /// An unexpected structural validation error from the protocol.
    #[error("unexpected constraint validation error: {0}")]
    Unknown(String),
}
...
pub fn check_credentials_against_proof_request(
    ...
    request.check_credentials(store, now)
}

#[uniffi::export]
impl ProofRequest {
    /// Checks local credential availability and time requirements for each request
    /// item, then evaluates the request's constraints without generating a proof.
    ///
    /// # Errors
    /// Returns an error if constraint validation or reading the credential store fails.
